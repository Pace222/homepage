...
export default function Component({ service }) {
  ...
  if (!piholeData) {
    return (
      <Container service={service}>
        <Block label="protectedpihole.queries" />
        <Block label="protectedpihole.blocked" />
        <Block label="protectedpihole.blocked_percent" />
        <Block label="protectedpihole.gravity" />
      </Container>
    );
  }
  ...
  return (
    <Container service={service}>
      <Block label="protectedpihole.queries" value={t("common.number", { value: parseInt(piholeData.dns_queries_today, 10) })} />
      <Block label="protectedpihole.blocked" value={blockedValue} />
      <Block
        label="protectedpihole.blocked_percent"
        value={t("common.percent", { value: parseFloat(piholeData.ads_percentage_today).toPrecision(3) })}
      />
      <Block
        label="protectedpihole.gravity"
